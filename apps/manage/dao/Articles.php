...
class Articles
{
    ...
    public function listArticles($data = array())
    {
        ...
        if (!empty($data))
        {
            ...
            $para['where']      = ' 1 ';
            if (isset($data['where'])) $para['where']  .= ' AND ' . $data['where'];
            if ('' != $start_time) $para['where']  .= ' AND a_start_time >=' . $start_time;
            if ('' != $end_time) $para['where']  .=' AND a_start_time <= ' . $end_time;
            if ('' != $a_title) $para['where']  .=' AND a_title LIKE "%' . $a_title . '%"';
            if ('' != $a_info) $para['where']  .=' AND a_info LIKE "%' . $a_info . '%"';
            if ('' != $a_link) $para['where']  .=' AND a_link LIKE "%' . $a_link . '%"';
            $para['page']          = isset($data['page']) ? $data['page'] : 1;
            $para['pager']      = isset($data['pager']) ? $data['pager'] : true;
            $para['pagesize']   = isset($data['pagesize']) ? $data['pagesize'] : PAGESIZE;
