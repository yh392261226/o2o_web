...
//print_r($_REQUEST);exit;
    ...
    private function price()
    {
            ...
            isset($data['o_worker']) && $data['o_worker'] > 0)
        {
            ...
            if (!empty($task_data['data'][0]))
            {
                ...
                //改价前后差价 = 原价 - 改后价
                $tmp['agio'] = ($tmp['original_amount'] - $data['tew_price']) * getDays($data['start_time'], $data['end_time']);
                if ($tmp['agio'] < 0)
                {
                ...
                //3：改数据库中的数据
                //任务更新改价次数及总任务改后价
                $tmp['edit_amount'] = ($data['tew_price'] * $data['worker_num'] * getDays($data['start_time'], $data['end_time']));
                ...
                //4：多退少补 操作平台与用户资金
                $user_funds_result = $this->userFunds($data['t_author'], $tmp['agio'], $type = 'changeprice'); //用户资金
                $platform_funds_result = $this->platformFundsLog($tmp['t_id'], (-1 * $tmp['agio']), 3, 'changeprice');     //平台资金日志
    ...
    private function unbind()
    {
        ...
        if (!empty($data) && isset($data['tew_id']) && isset($data['t_id']) && isset($data['o_worker']) && isset($data['s_id']) && isset($data['u_id']))
        {
            ...
            $order_param = $data;
            $order_param['pager'] = 0;
            $order_param['o_status'] = 0;
            $order_param['o_confirm'] = 1;
    ...
    private function cancel()
    {
        ...
        $data['where'] = 1;
        if (isset($_REQUEST['o_confirm']) && trim($_REQUEST['o_confirm']) != '') $data['where'] .= ' and o_confirm in ('. trim($_REQUEST['o_confirm']) .')'; //array('type' => 'in', 'value' => trim($_REQUEST['o_confirm']));
        $data['o_status'] = 0;
        ...
        if (!empty($data) && (isset($data['o_id']) || (isset($data['tew_id']) && isset($data['t_id']) && isset($data['u_id']) && isset($data['o_worker']) && isset($data['s_id']))))
        {
            ...
            $update_param = array('o_id' => $data['o_id'], 't_id' => $data['t_id']);
            $result = $this->orders_dao->updateData(array('o_status' => -4), $update_param);
            if ($result)
            {
...
//                $order_param['where'] = 'orders.o_confirm in (1, 2) and task_ext_worker.tew_status = 0 and orders.o_pay = 0';
                $order_param['where'] = 'orders.o_confirm in (1, 2) and orders.o_pay = 0';
                ...
                $order_param['where'] .= ' and orders.t_id = "' . intval($data['t_id']) . '" and orders.u_id = "' . intval($data['t_author']) . '"';
                if (isset($data['tew_id']))
                {
                    $order_param['where'] .= ' and orders.tew_id = "' . intval($data['tew_id']) . '" and orders.o_status in (0, -1, -2) ';
                }
                $order_param['pager'] = 0;
                ...
                {
                    //原该花费的总价
                    $origin_cost_total = $orders_data['data'][0]['tew_worker_num'] * $orders_data['data'][0]['tew_price'] * getDays($orders_data['data'][0]['tew_start_time'], $orders_data['data'][0]['tew_end_time']);
                    ...
                    foreach ($orders_data['data'] as $key => $val)
                    {
                            ...
                            isset($val['o_pay']) && $val['o_pay'] == 0)
                        {
                            ...
                            //订单中原有该花总价
                            $original_amount = $val['o_amount'] * getDays($val['tew_start_time'], $val['tew_end_time']);
                            $origin_cost_total = $origin_cost_total - ($val['tew_price'] * getDays($val['tew_start_time'], $val['tew_end_time'])) + $original_amount; //订单修改后的当前工种的总价

                            //解决辞职或解雇的工人价格
                            if ($val['o_status'] == -1) //辞职
                            {
                                //实际总价
                                $real_total = $val['o_amount'] * (getDays($val['tew_start_time'], $val['unbind_time']) - 1);
                                ...
                                //    $user_funds_result = $this->userFunds($val['u_id'], ($original_amount - $real_total), 'overage'); //雇主用户资金收入
                            ...
                            {
                                //实际总价
                                $real_total = $val['o_amount'] * getDays($val['tew_start_time'], $val['unbind_time']);
                                //if ($original_amount > $real_total)
                                //{
                                //    $platform_result = $this->platformFundsLog($val['o_id'], (($original_amount - $real_total) * -1), 0, 'taskreturn'); //平台资金支出
                                //    $user_funds_result = $this->userFunds($val['u_id'], ($original_amount - $real_total), 'overage'); //雇主用户资金收入
                    ...
                    if ($pay_status == 1)
                    {
                        ...
                        $platform_result = $this->platformFundsLog(intval($data['t_id']), ($origin_cost_total * -1), 3, 'taskreturn'); //平台资金支出
                        $user_funds_result = $this->userFunds(intval($data['t_author']), $origin_cost_total, 'overage'); //雇主用户资金收入
