...
class Tasks extends \CLASSES\WebBase
{
    ...
    private function info()
    {
        ...
        if (!empty($info))
        {
            ...
            if ($if_unite_phone) {
                $info['t_phone'] = $if_unite_phone;
            }
            $this->exportData($info);
        }
        else
        {
            $this->exportData(array('result' => "0"));
        }
    }
...
//        error_log(var_export($request_data, true) . '\n', 3, 'request.log');
        ...
        if (isset($request_data['worker']) && !empty($request_data['worker']))
        {
            ...
            foreach ($request_data['worker'] as $key => $val)
            {
                ...
                $worker[$key][10] = 0;
                $tmp['total'] = $tmp['total_edit'] += $worker[$key][2] * $worker[$key][3] * getDays($worker[$key][4], $worker[$key][5]);
                if ($tmp['total'] <= 0)
                {
...
//            if ($amount_result && $user_funds_result && $platform_funds_result)
    ...
    private function _delTaks($data = array())
    {
        ...
        if (isset($data['t_id']) && isset($data['t_author']))
        {
            ...
            if (!empty($del_info))
            {
                if (isset($del_info['t_author']) && $del_info['t_author'] == intval($data['t_author']))
                {
                    ...
                    $orders_data = $orders_dao->listData(array(
                        ...
                        'where' => 'o_status in (-3, -2, -1, 0) and o_pay = 0',
                    ));
                    if ((isset($del_info['t_status']) && $del_info['t_status'] != 0 && $del_info['t_status'] != 1) || !empty($orders_data['data']))
                    {
                        return -5; //任务已开始 不能删除
                    }
                }
            ...
            $order_dao->delOrders(array('t_id' => $data['t_id'], 'u_id' => $data['t_author'], 'o_status' => 0));
